...
function hospitalAPI() {
    return __awaiter(this, void 0, void 0, function* () {
        console.log("called");
        try {
            const host1 = 'http://apis.data.go.kr/B552657/HsptlAsembySearchService/getHsptlMdcncListInfoInqire'; // 모든 병원 데이터 가져옴
            const host2 = 'http://apis.data.go.kr/B552657/HsptlAsembySearchService/getHsptlBassInfoInqire'; // 한 병원 데이터 가져옴
            const SERVICE_KEY = 'I2F%2B1Oce6drCgGSm33cvy%2F3uLnHQ4BY46ALKDYUbKqPqslTOBJTUzx1yH%2FPt%2FsnttC0mZeVuTudJWDJ70xLCnw%3D%3D';
            const queryString1 = `?Q0=${encodeURIComponent("서울특별시")}&Q1=${encodeURIComponent("성북구")}&pageNo=1&numOfRows=1000&ServiceKey=${SERVICE_KEY}`;
            const requestUrl1 = host1 + queryString1;
            const result1 = yield request_promise_native_1.default.get(requestUrl1); // <- 요청 보냄
            ...
            for (const key in keys) {
                const queryString2 = `?HPID=${hospitalArr[key].hpid._text}&ServiceKey=${SERVICE_KEY}`;
                const requestUrl2 = host2 + queryString2;
                const result2 = yield request_promise_native_1.default.get(requestUrl2);
                ...
                hspiCategory = hspiCategory.filter((item, index) => {
                    return hspiCategory.indexOf(item) === index;
                });
                let arr = Object.keys(hospitalArr[key]); // 병원 목록에 들어있는 키들.
                let arr2 = [];
                for (const x of arr) {
                    if (x.slice(0, 8) === "dutyTime")
                        arr2.push(x);
                    if (x.slice(0, 5) === "wgs84")
                        arr2.push(x);
                    if (x.slice(0, 7) === "dutyInf")
                        arr2.push(x);
                    if (x.slice(0, 10) === "dutyMapimg")
                        arr2.push(x);
                }
                const obj = {};
                const obj2 = [
                ...
                for (var s = 1; s < obj2.length; s += 2) {
                    if (arr2.indexOf(obj2[s]) !== -1) {
                        if (obj2[s] === "wgs84Lat") {
                            ...
                            obj[obj2[s]] = `${hospitalArr[key][obj2[s]]._text.slice(0, 2)}:${hospitalArr[key][obj2[s]]._text.slice(2, 4)}`;
                        }
                    }
                    else {
                ...
                for (var c = 0; c < obj2.length; c += 2) {
                    if (arr2.indexOf(obj2[c]) !== -1) {
                        if (obj2[c] === "wgs84Lon") {
                            ...
                            obj[obj2[c]] = `${hospitalArr[key][obj2[c]]._text.slice(0, 2)}:${hospitalArr[key][obj2[c]]._text.slice(2, 4)}`;
                        }
                    }
                    else {
                ...
                hospitalList = yield Hospital_model_1.default.create({
                    // <- Hospital 테이블에 로우 생성
                ...
                yield hospitalList.$add("category", hspiCategory);
            }
        }
        catch (error) {
            console.error(error);
...
//# sourceMappingURL=hospitalApi.util.js.map